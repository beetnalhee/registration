...
| 참가 연령 | 만 18~35세 |
| 회차 선택 | **1개만 선택 (선착순)** — 고른 회차가 마감이면 그 회차 대기자 |
...
- 남녀 인원수, 잔여석, 신청 인원
- 그룹의 연령 구간
- Bridge Zone(경계 연령 참가자를 성비 보정용으로 이동시키는 내부 규칙)
...
> `004` 는 희망 회차 컬럼(`pref_1~3`)을 단일 컬럼으로 옮기므로,
> 기존 신청 데이터가 있으면 1순위 값이 선택 회차로 이관됩니다.
...
- 40명이 동시에 같은 회차를 신청 → **정확히 20명만 배정**, 나머지는 그 회차 대기자 (다른 회차로 넘기지 않음)
- 정원 초과분은 모두 대기자, 어떤 회차도 20명을 넘지 않음
- 남녀 정원이 서로를 잠식하지 않음
- 참가번호 중복 없음, 취소된 번호 재사용 없음
- 취소 시 좌석 반납 → 다음 사람이 들어갈 수 있음
